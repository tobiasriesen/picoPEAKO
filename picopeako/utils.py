...
def mask_velocity_vectors(spec_data: list):
    ...
    :param spec_data: list of xarray DataSets containing Doppler spectra, and the variable
                      velocity_vectors
    :return:
    """
    for i in range(len(spec_data)):
        np.putmask(
            spec_data[i].velocity_vectors.values, spec_data[i].velocity_vectors.values > 9000,
            np.nan
        )
    return spec_data
...
def save_and_reload(spec_data, filenames):
    """
    Following optimization tip #3 (https://xarray.pydata.org/en/v2023.04.2/user-guide/dask.html)
    save intermediate results to disk and then load them again
    ...
    spec_data = [xr.open_dataset(i, mask_and_scale=True, chunks={"time": 10}) for i in filenames]
    return spec_data
...
def get_closest_time(time, time_array):
    ...
    if (time_array < 1e9).all() and (time_array > 3e8).all():
        time_array += (datetime.datetime(2001, 1, 1) - datetime.datetime(1970, 1, 1)).total_seconds()
